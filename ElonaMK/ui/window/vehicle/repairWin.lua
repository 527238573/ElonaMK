...
local repairWin = ui.new_window()
ui.vehicleWin.repairWin = repairWin



local blockScreen_id = newid()
local s_win = {name = tl("修复组件","Repair component"),x= c.win_W/2-380,y =c.win_H/2-300, w= 560,h =497,dragopt = {id= newid()}}
...
local function loadCondition()
  ...
  local totally_destory = cur_component.hp<=0 
  ...
  if not totally_destory then requireLevel = requireLevel+2 end --修复加2级
  ...
  local text3
  local costTime = (ctype.install_time/2) --移除时间是安装时间的2分之1而
  if totally_destory then costTime = ctype.install_time*1.1 end 
...
local do_repair --提前声明
local repair_opt = {id= newid(),text = tl("修复[e/Enter]","Repair[e/Enter]"),font=c.font_c16}
local cancel_opt = {id= newid(),text = tl("取消[q/Esc]","Cancel[q/Esc]"),font=c.font_c16}
local function buttonlist(x,y)
  if curEntry ==nil then return end
  repair_opt.disable = not curEntry.meet_all
  local remove_state  = suit:S9Button(repair_opt.text,repair_opt,x,y,150,35) 
  local cancel_state  = suit:S9Button(cancel_opt.text,cancel_opt,x+200,y,150,35) 
  
  if cancel_state.hit then repairWin:Close()  end
  if remove_state.hit then do_repair()  end
end
...
function do_repair()
  ...
  activity.name = string.format(tl("修复组件: %s","Repairing component: %s"),ctype.name)
  ...
  local function complete_activity(activity)
    ...
    addmsg(string.format( tl("你修理好了%s的%s。", "You repair the %s's %s."),cur_veh.name,ctype.name),"info")
    --重开
    ui.vehicleWin:Open(cur_veh)--重打开
  end
  --中断函数。
  local function cancel_activity(activity)
    addmsg(string.format( tl("你停止修理%s。", "You stop repairing %s."),ctype.name),"info")
...
function repairWin.keyinput(key)
  if key=="escape"or key =="q" then  repairWin:Close() end
  if key == "return" or key == "e" then do_repair() end
end

--传输进来的组件肯定是unmount检查过的。
function repairWin.win_open(vehicle,component)
...
function repairWin.win_close()

end
function repairWin.window_do()
  ...
  if close_st.hit then repairWin:Close() end
end