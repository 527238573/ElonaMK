...
function ui.numberAsk_keyinput(key)
  if key=="e" or key=="return" then winClose() end
  if key=="q" or key=="escape" then winClose(true) end
end


--仅仅是修改初始状态
function ui.numberAskOpen(endcall,min,max,defalut_value,x,y,nameinfo)
  callback = endcall
  slider_info.min = min
  slider_info.max = max
...
function ui.numberAsk()
  ...
  if ok_st.hit then winClose()end
  if ce_st.hit then winClose(true)end
end