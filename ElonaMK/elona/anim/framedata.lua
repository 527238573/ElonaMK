...
--自动切割
local function load1f(fileid,w,h,secPerFrame,scaleFactor,ox,oy,path)
  path = path or fileid
  w=w or 32;h=h or 32
  ...
  --自动切割
  local img = love.graphics.newImage("data/frames/"..path..".png")
...
local function loadAbi(fileid,w,h,secPerFrame,scaleFactor,ox,oy)
  local path = "ability/"..fileid
  load1f(fileid,w,h,secPerFrame,scaleFactor,ox,oy,path)
end
...
local function loadFrames()
  ...
  loadAbi("ancient_wisdom",64,64,0.05)
